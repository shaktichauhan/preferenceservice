...
<link rel="STYLESHEET" type="text/css" href="css/onepass/onePassKindle.css" />

<%
		String sourceId = (String) request.getParameter("sourceId");

		if(sourceId == null || "".equals(sourceId.trim())) {
			sourceId = "770"; // default kindle RD source id
		}
		
	  	String addressToggle = StringUtils.getBundleProperty("com.readersdigest.onepass.OnePassResources", "addressToggle");
	  	
	  	String accountNumber = request.getParameter("accountNumber") != null?(String)request.getParameter("accountNumber"):"";
		...
		String confirmEmailAddress = (String)request.getParameter("confirmEmailAddress");
		String token = (String)request.getParameter("token");
		String country = (String)request.getParameter("country");
		String confirmPassword = (String)request.getParameter("confirmPassword") != null?(String)request.getParameter("confirmPassword"):"";
		...
		String transactionCompleted = (String)request.getParameter("transactionCompleted");
...
<body onload='selectedCountry();'>
	...
	<html:form action="onePassKindleRegistrationAction.do"
		styleId="onePassDigReg">
		<img src="image/readers-digest-logo.gif" width="220" height="102" border="0" />
		
		<h3>REGISTER</h3>
		<p>Create an account to stay up to date and access future free benefits, including bonus content on your device.</p>
		...
		<fieldset>
			...
			<ol>
				<li><label for=email>Email Address</label> <html:text
						property="emailAddress" value="<%=emailAddress%>" maxlength="100"></html:text></li>

				<li><label for=confirmemail>Confirm Email Address</label> <html:text
						property="confirmEmailAddress" value="<%=confirmEmailAddress%>" maxlength="100"></html:text></li>
				...
				<li><label for=confirmpassword>Confirm Password</label>
				 	<input type="password" id="confirmPassword" name="confirmPassword" value="<%=confirmPassword%>" maxlength="50" />
				 </li>
		...
		<% if("true".equalsIgnoreCase(addressToggle)) {%>
			...
			<ol>
				...
				<li><label for=accountNumber>Account Number</label>
				 	<input type="text" id="accountNumber" name="accountNumber" value="<%=accountNumber%>" placeholder="For Print Subscribers only" maxlength="50" />
				 </li>
			</ol>
		...
		<fieldset>
			...
			<input type="hidden" name="promotable_unchecked" value="10:11" id="promotable_unchecked" />
			<ol class="optbox">
			
			    <li><input type="checkbox" name="promotable" value="10" id="promotable_field" checked="checked" />Send me occasional e-mail updates and special offers from Reader's Digest.<br />
                    <input type="checkbox" name="promotable" value="11" id="promotable_field" checked="checked"/>Get special offers from third parties whom we've specially selected just for you.<br /><br />
			...
			<% } else { %>
			<input type="hidden" name="promotable_unchecked" value="20:30" id="promotable_unchecked" />
			<ol class="optbox">
		...
		<fieldset>
			...
			%>
			<h4 align='center'>
				<font color='#dd3937'>Your account has been created  - go sign in!</font>
				<% if("true".equalsIgnoreCase((String)request.getParameter("confirmationRequired"))) { %>
     				<br /><br /><font color='#dd3937'>A confirmation email has been sent to your email address. Click on the confirmation link in the email to access complimentary digital rd publication.
   				 </font>
   				 <% } %>
			</h4>
			<%
